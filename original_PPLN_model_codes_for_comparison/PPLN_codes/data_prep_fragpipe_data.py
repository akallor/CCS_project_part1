...
# Load the data
print("Loading data...")
train_data = pd.read_csv("/content/drive/MyDrive/Colab_CCS_results/MHC_1/New_data_jun2025/combined_ion.tsv", 
                        sep='\t')

# Filter out for MHC-I or II based on requirement
train_data = train_data[(train_data['Peptide Length'] <= 12) & (train_data['Charge'] == 1)]
...
# Define the B-columns for dataset and invK0
b_columns = ['B1_1 Apex Ion Mobility', 'B2_1 Apex Ion Mobility',
             'B3_1 Apex Ion Mobility', 'B4_1 Apex Ion Mobility',
             'B4_2 Apex Ion Mobility', 'B5_1 Apex Ion Mobility',
             'B6_1 Apex Ion Mobility', 'B7_2 Apex Ion Mobility',
             'B8_1 Apex Ion Mobility', 'B8_2 Apex Ion Mobility',
             'B8_3 Apex Ion Mobility', 'B9_1 Apex Ion Mobility']

# Create dataset mapping
dataset_mapping = {
    'B1_1 Apex Ion Mobility': 'B1',
    'B2_1 Apex Ion Mobility': 'B2', 
    'B3_1 Apex Ion Mobility': 'B3',
    'B4_1 Apex Ion Mobility': 'B4',
    'B4_2 Apex Ion Mobility': 'B4_2',
    'B5_1 Apex Ion Mobility': 'B5',
    'B6_1 Apex Ion Mobility': 'B6',
    'B7_2 Apex Ion Mobility': 'B7_2',
    'B8_1 Apex Ion Mobility': 'B8',
    'B8_2 Apex Ion Mobility': 'B8_2',
    'B8_3 Apex Ion Mobility': 'B8_3',
    'B9_1 Apex Ion Mobility': 'B9'
}

# Select required columns and add derived columns
required_columns = ['Peptide Sequence', 'M/Z', 'Peptide Length', 'Charge']
...
data_subset['Mass'] = data_subset['M/Z'] * data_subset['Charge']
...
# Expand data to create one row per peptide per dataset
print("Expanding data to create one row per peptide per dataset...")
expanded_rows = []

for idx, row in data_subset.iterrows():
    peptide_data = row.to_dict()
    
    # Check each B-column for valid values
    for b_col in b_columns:
        if b_col in train_data.columns:
            invk0_value = train_data.loc[idx, b_col]
            
            # Only include if invK0 > 0 (valid measurement)
            if pd.notna(invk0_value) and invk0_value > 0:
                # Create new row for this dataset
                new_row = peptide_data.copy()
                new_row['Dataset'] = dataset_mapping[b_col]
                new_row['invK0'] = invk0_value
                expanded_rows.append(new_row)

# Create expanded dataframe
data_expanded = pd.DataFrame(expanded_rows)

print(f"Data shape after expansion: {data_expanded.shape}")

# Remove rows with missing values after expansion
data_expanded = data_expanded.dropna()
print(f"Data shape after removing NaN in expanded data: {data_expanded.shape}")

# Handle duplicate sequences within same dataset
print("Handling duplicate sequences within datasets...")
data_clean = data_expanded.groupby(['Peptide', 'Dataset']).agg({
    'invK0': 'mean',    # Average invk0 values for same peptide in same dataset
    'Charge': 'first',  # Assuming charge is consistent for same sequence
    'Mass': 'first',    # Assuming mass is consistent for same sequence
    'Length': 'first'   # Assuming length is consistent for same sequence
}).reset_index()
...
print(train_data_split['Dataset'].value_counts().sort_index())
print(f"\nDataset distribution in test set:")
print(test_data_split['Dataset'].value_counts().sort_index())
