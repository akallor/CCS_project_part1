#!/usr/bin/env python3
"""
Unified ESM-2 + RNN CCS Training and Evaluation
================================================

- Loads precomputed ESM-2 + engineered features from .pt (from charge_aware_esm
  feature extraction) and aligns with TSV (Sequence, Charge, CCS_Experimental).
- Stratified train/val/test split by charge (same as unified predictor).
- Trains UnifiedESMCCSPredictor (2-layer RNN on ESM + chemical features + charge).
- Loss: MSE or Huber; AdamW; warmup + cosine LR; early stopping.
- Evaluation stratified by charge; saves metrics and plots.

Usage:
  1. Extract features on your TSV (same order as data):
     Use charge_aware_esm_feature_extraction or run_extract_esm_for_unified.py
  2. Train:
...
class UnifiedESMTrainer:
    ...
    def validate(
        ...
        for esm, eng, charge, target in val_loader:
            esm = esm.to(self.device, non_blocking=non_blocking)
            eng = eng.to(self.device, non_blocking=non_blocking)
            charge = charge.to(self.device, non_blocking=non_blocking)
            target = target.to(self.device, non_blocking=non_blocking)
...
def plot_charge_stratified(
    ...
    # Per-charge plots: Experimental vs Predicted, Experimental vs Residual, Residual distribution
    for ch in ch_list:
        mask = charges == ch
        t_ch = targets[mask]
        p_ch = preds[mask]
...
def main():
    parser = argparse.ArgumentParser(description="Unified ESM-2 + RNN CCS training")
    ...
    parser.add_argument("--output_dir", type=str, default="./unified_esm_results")
