...
Usage:
  1. Extract features: run_extract_esm_for_unified.py (optionally run_extract_esm_for_unified_hla1.py / _hla2.py)
  2. Train:
  ...
  HLA by length (same TSV + HLA-specific .pt from split extraction):
     python run_unified_esm_training_corrected_hla1.py --data_path ... --features_path features_hla1.pt --output_dir ...
     python run_unified_esm_training_corrected_hla2.py --data_path ... --features_path features_hla2.pt --output_dir ...

  Optional explicit length filter (requires Sequence column): --min_length / --max_length

  HPC / "Too many open files": if DataLoader raises RuntimeError, use --num_workers 0 and/or
  omit --persistent_workers (default), raise the shell limit (ulimit -n), or rely on the
  built-in torch.multiprocessing file_system sharing strategy set at import.
"""
...
def load_features_and_tsv(
    ...
    If the .pt contains ``selected_indices`` (e.g. from HLA-specific extraction), the TSV
    is subset to those rows before alignment. Optional ``min_length`` / ``max_length`` filter
    the aligned rows (requires ``sequence_column`` in the TSV when either bound is set).
    """
    ...
    if min_length > 0 or max_length > 0:
        if sequence_column not in df.columns:
            raise ValueError(
                f"Length filtering requires '{sequence_column}' in TSV. Got {list(df.columns)}"
            )
        ...
        if kept == 0:
            raise RuntimeError(
                f"No samples remain after length filtering with min={min_length}, max={max_length}"
            )
        keep_t = torch.from_numpy(keep.astype(np.bool_))
        esm_tensor = esm_tensor[keep_t]
        eng_tensor = eng_tensor[keep_t]
        charges = charges[keep]
        targets = targets[keep]
        print(
            f"Length filter applied: min={min_length}, max={max_length}; "
            f"kept {kept}/{n} aligned samples"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="DataLoader workers; use 0 on HPC if you still hit 'Too many open files'",
    )
    parser.add_argument(
        "--persistent_workers",
        action="store_true",
        help="Keep DataLoader workers between epochs (faster; uses more file descriptors)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Minimum peptide length to include after alignment (0 = no filter). Requires Sequence in TSV.",
    )
    parser.add_argument(
        "--max_length",
        type=int,
        default=0,
        help="Maximum peptide length to include (0 = no upper bound). Requires Sequence in TSV.",
    )
    ...
    # Per-charge split counts (1-based charge labels) — helps explain per-charge performance
