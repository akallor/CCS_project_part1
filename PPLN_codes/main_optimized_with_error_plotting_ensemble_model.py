    ...
    for batch_idx, (charge_mass, seq, ccs) in enumerate(train_loader):
        ...
        if config.model_type == 'ensemble':
            ...
            if batch_idx % 10 == 0:  # Log components every 10 batches
                print(f"\nLoss components - Main: {main_loss:.4f}, Diversity: {diversity_loss:.4f} (weight: {diversity_weight:.4f})")
        else:
        ...
        # Record metrics using main loss for ensemble
        if config.model_type == 'ensemble':
            total_loss += main_loss.item() * len(charge_mass)
        else:
        ...
        if isinstance(output, tuple):
            epoch_preds.append(output[0].detach())
        else:
...
def validate_both(models, val_loader, criterion, device, normalizer=None):
...
def validate_combined(improved_model, ensemble_model, val_loader, criterion, device, normalizer=None):
    improved_model.eval()
    ensemble_model.eval()
    total_loss = 0
    all_preds = []
